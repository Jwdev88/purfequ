...
const Product = () => {
  ...
  if (!state.productData) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-2xl font-semibold">Product data not available.</p>
      </div>
    );
  }

  const categoryName = state.productData.category?.name || "Unknown Category";
  const subCategoryName =
    state.productData.subCategory?.name || "Unknown Subcategory";
